-- ============================================================
-- PHASE 5.3 — CORRECTIVE MIGRATION
-- Name: phase_5_3_semantic_search_fix
-- Fixes:
--   1. ERROR 42601: LIMIT inside UNION ALL members in autocomplete_seafood()
--      → Each branch with LIMIT is now wrapped in a subquery (SELECT * FROM (...) sub)
--   2. dt.name → dt.label in search_seafood_knowledge() documents branch
--      → document_types table uses column "label", not "name"
-- This migration is idempotent (CREATE OR REPLACE / IF NOT EXISTS).
-- Does NOT replay any previous migration.
-- Does NOT drop or modify existing tables or data.
-- ============================================================
...
-- ============================================================
-- 3. RLS ON ANALYTICS TABLES (idempotent)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_species_fts
  ON public.species USING gin (
    to_tsvector('simple',
      ...
      coalesce(family,'') || ' ' || coalesce(description,''))
  );
...
CREATE INDEX IF NOT EXISTS idx_assets_fts
  ON public.assets USING gin (
    to_tsvector('simple',
      ...
      coalesce(product_form,'') || ' ' || coalesce(product_state,''))
  );
...
-- ============================================================
-- 5. HELPER: normalize_search_text (idempotent)
-- ============================================================
...
--    FIX: dt.name → dt.label in documents branch
-- ============================================================
...
BEGIN
  ...
  WITH candidates AS (
    ...
    -- FIX: dt.name → dt.label (document_types table uses column "label")
    SELECT
      'document'::text,
      d.id,
      NULL::text,
      d.public_title,
      dt.label,                          -- ← FIXED: was dt.name
      NULL::text,
      CASE
    ...
    -- ── LEVEL 4: Knowledge Graph entities ────────────────────────────────
    SELECT
...
-- ============================================================
-- 7. AUTOCOMPLETE RPC: autocomplete_seafood
--    FIX: LIMIT inside UNION ALL members is illegal in PostgreSQL.
--         Each branch is now wrapped in a subquery so LIMIT is applied
--         before the UNION ALL, which is valid SQL.
-- ============================================================
...
BEGIN
  ...
  WITH ac AS (
    ...
    -- Species branch — wrapped in subquery so LIMIT is valid
    SELECT * FROM (
      SELECT
        'species'::text                                        AS ot,
        s.id                                                   AS id,
        s.slug                                                 AS slug,
        s.common_name                                          AS t,
        s.scientific_name                                      AS st,
        s.is_validated                                         AS iv,
        CASE
          WHEN normalize_search_text(s.common_name) LIKE v_norm || '%' THEN 1.0
      ...
      WHERE s.is_demo = false
        AND (
          s.common_name ILIKE query_text || '%'
          OR s.common_name ILIKE '%' || query_text || '%'
    ...
    -- Products branch — wrapped in subquery so LIMIT is valid
    SELECT * FROM (
      SELECT
        'product'::text,
        cp.id,
      ...
      ORDER BY (cp.status = 'verified') DESC
      LIMIT 3
    ) product_branch

    UNION ALL

    -- Markets branch — wrapped in subquery so LIMIT is valid
    SELECT * FROM (
      SELECT
        'market'::text,
        m.id,
      ...
      ORDER BY (m.status = 'verified') DESC
      LIMIT 2
    ) market_branch

    UNION ALL

    -- Certifications branch — wrapped in subquery so LIMIT is valid
    SELECT * FROM (
      SELECT
        'certification'::text,
        c.id,
      ...
      ORDER BY (c.status = 'verified') DESC
      LIMIT 2
    ) certification_branch

    UNION ALL

    -- Species names branch — wrapped in subquery so LIMIT is valid
    SELECT * FROM (
      SELECT
        'species_name'::text,
        s.id,
...
-- ============================================================
-- 8. FUZZY SUGGESTION RPC (Did you mean?) — idempotent
-- ============================================================
...
-- ============================================================
-- 10. ADMIN ANALYTICS VIEW — idempotent
-- ============================================================
