...
function ProductCard({ product }: { product: EncProduct }) {
  ...
  return (
    ...
    >
      <div className="h-24 bg-gradient-to-br from-ocean-900 to-ocean-700 flex items-center justify-center relative">
      ...
      <div className="p-4">
        <h3 className="font-semibold text-foreground text-sm leading-tight mb-1">{product.public_name}</h3>
        {product.description && (
...
export default function ProductsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const result = await fetchEncProductList({ page, pageSize: PAGE_SIZE, search: debouncedSearch, verifiedOnly });
    setProducts(result.data);
    setTotal(result.total);
    setLoading(false);
  }, [page, debouncedSearch, verifiedOnly]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-screen-2xl mx-auto w-full px-4 lg:px-8 xl:px-10 2xl:px-16 pt-24 pb-16">
        ...
        <div className="mb-8">
          <p className="text-xs font-semibold uppercase tracking-widest text-secondary mb-2">Product Center</p>
          <h1 className="text-3xl font-bold text-foreground mb-2">Commercial Seafood Products</h1>
          <p className="text-muted-foreground max-w-2xl leading-relaxed text-sm">
            Commercial products linked to species, presentations, processing methods, preservation, packaging and markets.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors ${showFilters || verifiedOnly ? 'bg-secondary/10 border-secondary/40 text-secondary' : 'bg-card border-border text-muted-foreground hover:text-foreground'}`}
          >
        ...
        {showFilters && (
          <div className="bg-card border border-border rounded-xl p-4 mb-6 flex flex-wrap gap-4 items-center">
            <div className="flex items-center gap-2">
              <input type="checkbox" id="prod-verified" checked={verifiedOnly} onChange={(e) => { setVerifiedOnly(e.target.checked); setPage(1); }} className="w-4 h-4 accent-secondary" />
              <label htmlFor="prod-verified" className="text-sm text-foreground cursor-pointer">Verified only</label>
            </div>
            {verifiedOnly && (
              <button onClick={() => setVerifiedOnly(false)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                <X size={12} /> Clear
              </button>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card rounded-2xl border border-border">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">
              {debouncedSearch ? 'No products match your search' : 'Product catalog building in progress'}
            </h3>
            <p className="text-sm text-muted-foreground max-w-sm mx-auto">
              {debouncedSearch ? 'Try a different search term.' : 'Commercial products are being documented and validated.'}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
